...
AQueenActor::AQueenActor()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> FoundMesh(TEXT("StaticMesh'/Game/Meshes/Queen.Queen'"));
...
*/
void AQueenActor::DetectSelectableGrids(TArray<UStaticMeshComponent*> *SelectableGrids, TArray<UMaterial*> *DefaultMaterials)
{
	...
	// The same with rook selectables.
	for (int32 i = 0; i < 4; i++)
	{
		...
		while (1)
		{
			...
			/*Checking if anyting was overlapped.*/
	...
	ChangeXBy = 400.0f, ChangeYBy = 400.0f; // To update X and Y value by this.

	// The same with bishop selectables.
	for (int32 i = 0; i < 4; ++i)
	{
		SpawnLocation = FVector(GetActorLocation().X, GetActorLocation().Y, 0); // Reset spawn value.
...
*/
void AQueenActor::AddAndHighlight(TArray<UStaticMeshComponent*> *SelectableGrids, TArray<UMaterial*> *DefaultMaterials, TArray<AActor*> *OverlappedActors, TArray<UPrimitiveComponent*> *OverlappedComponents)
{
